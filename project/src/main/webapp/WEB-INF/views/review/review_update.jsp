...
<body>
	...
	<form action="reviewform.do" method="post">
		...
		<input type="text" name="review_writer_id" value="kakao_1122334455"
			hidden=hidden> <input type="text" name="review_foodstore_seq"
			value="2" hidden=hidden> <input type="text"
			name="review_good_number" value="0" hidden=hidden>
	</form>

	<c:if test="${aList != null}">
		<c:forEach var="dto" items="${aList}" varStatus="status">
			<input type="text"
				id='review_foodstore_seq${aList[status.index].review_foodstore_seq}'
				name="review_foodstore_seq"
				value="${aList[status.index].review_foodstore_seq}" hidden=hidden>
			<input type="text" name="review_seq"
				value="${aList[status.index].review_seq}" hidden=hidden>
				...
				<div class="card p-3 justify-content-start mt-2">
					<div class="row">
						<div class="card-title">
								...
								<h4>방문자 리뷰								<a
									href="review_update_form.do?review_edit_num=${aList[status.index].review_seq}"
									data-abc="true" id="edit${aList[status.index].review_seq}"><i
									class="fa fa-pencil fa-lg" alt="수정"></i></a><a href="review_delete.do?review_delete_num=${aList[status.index].review_seq}" alt="삭제">삭제</a></h4>
							...
							<div class="comment-widgets d-flex m-b-20">
								<div class="profile column-row mt-3">
									<div class="profile-image">
										<span class="round"><img
											...
											id="user_profile_image${aList[status.index].review_seq}" />
										</span>
									</div>
								...
								<div class="comment-inside p-3 col-2">
									<div class="comment-info column-row">
										<div class="user_info">
											<span id="user_nickname${aList[status.index].review_seq}">${aList[status.index].review_writer_id}</span>
										</div>
										...
										<div class="write_date">
											<span class="date" id="date${aList[status.index].review_seq}">${alist_write_date[status.index]}</span>
										</div>
								...
								<div class="comment-text mt-3">
									<p id="comment-text${aList[status.index].review_seq}">${aList[status.index].review_content}</p>
								</div>
							</div>
							<div class="row">
